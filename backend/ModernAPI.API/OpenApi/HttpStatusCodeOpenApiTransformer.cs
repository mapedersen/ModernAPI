...
public sealed class HttpStatusCodeOpenApiTransformer : IOpenApiDocumentTransformer
{
    ...
    /// <summary>
    /// Enhances existing ProblemDetails schemas or adds them if they don't exist.
    /// Works with auto-generated schemas from controller attributes and AddProblemDetails().
    /// </summary>
    ...
    private static void AddProblemDetailsSchemas(OpenApiDocument document)
    {
        ...
        // Only enhance existing schemas - don't add new ones to avoid conflicts
        if (document.Components.Schemas.TryGetValue("ProblemDetails", out var existingProblemDetails))
        {
            EnhanceProblemDetailsSchema(existingProblemDetails);
        }
        
        if (document.Components.Schemas.TryGetValue("ValidationProblemDetails", out var existingValidationProblemDetails))
        {
            EnhanceValidationProblemDetailsSchema(existingValidationProblemDetails);
        }
    }

    /// <summary>
    /// Enhances an existing ProblemDetails schema with additional ModernAPI-specific properties.
    /// </summary>
    /// <param name="existingSchema">The existing ProblemDetails schema to enhance</param>
    private static void EnhanceProblemDetailsSchema(OpenApiSchema existingSchema)
    {
        // Ensure the schema has properties collection
        existingSchema.Properties ??= new Dictionary<string, OpenApiSchema>();

        // Add ModernAPI-specific properties if they don't already exist
        existingSchema.Properties.TryAdd("requestId", new OpenApiSchema
        {
            Type = "string",
        ...
        existingSchema.Properties.TryAdd("timestamp", new OpenApiSchema
        {
            Type = "string",
            Format = "date-time",
        ...
        // Enhance the description if it's basic
        if (string.IsNullOrEmpty(existingSchema.Description) || 
            existingSchema.Description.Length < 50)
        {
            existingSchema.Description = "A machine-readable format for specifying errors in HTTP API responses based on RFC 7807, enhanced with ModernAPI-specific fields";
        }
    }

    /// <summary>
    /// Enhances an existing ValidationProblemDetails schema with additional properties and examples.
    /// </summary>
    /// <param name="existingSchema">The existing ValidationProblemDetails schema to enhance</param>
    private static void EnhanceValidationProblemDetailsSchema(OpenApiSchema existingSchema)
    {
        // Ensure the schema has properties collection
        existingSchema.Properties ??= new Dictionary<string, OpenApiSchema>();

        // Add or enhance the errors property with better documentation and examples
        if (existingSchema.Properties.TryGetValue("errors", out var errorsProperty))
        {
            // Enhance existing errors property
            errorsProperty.Description ??= "Validation errors grouped by field name";
            errorsProperty.Example ??= new Microsoft.OpenApi.Any.OpenApiObject
            {
        ...
        if (string.IsNullOrEmpty(existingSchema.Description))
        {
            existingSchema.Description = "A machine-readable format for specifying validation errors in HTTP API responses, extending the base ProblemDetails format";
        }
    }
