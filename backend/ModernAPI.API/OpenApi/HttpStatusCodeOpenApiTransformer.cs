...
public sealed class HttpStatusCodeOpenApiTransformer : IOpenApiDocumentTransformer
{
    ...
    /// Uses safe schema addition and enhancement to avoid conflicts with auto-generated schemas.
    /// </summary>
    ...
    private static void AddProblemDetailsSchemas(OpenApiDocument document)
    {
        ...
        // Enhanced RFC 7807 Problem Details schema
        // Use TryAdd for thread-safe, conflict-free addition
        ...
        // Use TryAdd to safely add schema without conflicts
        if (!document.Components.Schemas.TryAdd("ProblemDetails", problemDetailsSchema))
        {
            // If schema already exists, enhance it with our additional properties
            if (document.Components.Schemas.TryGetValue("ProblemDetails", out var existingSchema))
            {
                EnhanceProblemDetailsSchema(existingSchema);
            }
        }
        
        // Validation Problem Details schema (extends Problem Details)
        var validationProblemDetailsSchema = new OpenApiSchema
        {
        ...
        // Use TryAdd for ValidationProblemDetails as well
        document.Components.Schemas.TryAdd("ValidationProblemDetails", validationProblemDetailsSchema);
    }
